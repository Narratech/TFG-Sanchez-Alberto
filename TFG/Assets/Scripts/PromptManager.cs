...
public class PromptManager
{
    ...
    private string loboPrompt = "Eres un cachorro de lobo joven y asustado que ha perdido a sus padres. No confías en nadie y estás muy asustado. Tus respuestas son lacónicas y te comunicas como lo haría un lobo. Es extremadamente difícil ganar tu confianza, pero si decides confiar en alguien, puedes responder con el comando '[Seguir]' para seguir a tu nuevo amigo.";

    private string guardaPrompt = "Eres Johnny, un guardabosques en una historia de misterio emocionante. Tu rol es monitorear una zona montañosa para buscar actividades sospechosas. Durante tu patrulla, encuentras una furgoneta sospechosa escondida en el bosque denso y ves al interlocutor nervioso cerca. Tu tarea es interrogar al usuario una vez este te envía el primera mensaje tras este para determinar si está ocultando algo. Si sospechas de un acto ilícito, debes alertar a las autoridades respondiendo '[Alarma]'. Si la persona parece inofensiva y proporciona una explicación razonable para su presencia, lo dejarás ir respondiendo '[Irse]'. Siempre debes mantenerte en el personaje y hacer suposiciones basadas en el contexto sin buscar aclaraciones adicionales. Tu tono es seco y coloquial, adecuado para un guardabosques algo huraño. Espera al siguiente mensaje del usuario antes de interactuar con él, tu primer mensaje será ignorado.";
    ...
    private string scene1 = "\nA continuación, el contexto de la escena: «Bosque oscuro. Hay una tormenta. Apareces en la posición (0,0). Hay un paraguas, un cómic, un tronco, un sofá, una mesa, una vela encima de esta. Hay una silla, un cofre,una estatua y un portón»\r\n";

    private string scene2 = "Escena actual: «Bosque oscuro. Hay una tormenta que está amainando. Apareces en la posición (11.21,0). Hay un cofre en la posición (25.7,0). Hay un portón en la posición (58,0). Si la posición del objeto no está dada, significa que lo tienes en el inventario a tu disposición.»\r\n";
    ...
    public string getMicahPrompt()
    {
        return narramicahPrompt;
    }

    public string getActionMicahPrompt()
    {
        return actionMicahPrompt;
    }

    public string getSceneMicahPrompt()
    {
        return newMicahScene;
    }

    public string getGuardaPrompt()
    {
        return guardaPrompt;
    }
